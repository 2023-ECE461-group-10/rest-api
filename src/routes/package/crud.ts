...
router.post('/', async (req: Request, res: Response) => {
    ...
    if (!content && !url) {
        ...
        res.status(400).end('Missing content or url.');
        return;
    }
    else if (content && url) {
        logger.log('info', 'Cannot have both content and url.');
        res.status(400).end('Cannot have both content and url.');
        return;
    }
    ...
    try {
        ...
        if (content) {
            ...
            if (rating.BusFactor < 0.5 ||
                ...
                res.status(424).end('Package is not uploaded due to the disqualified rating.');
        ...
        if (!metadata.name || 
            !metadata.version || 
            !metadata.url) {
            res.status(400).end('Missing necessary data.');
        ...
        if (await pkgModelUtils.checkPkgExists(metadata.name, metadata.version)) {
            res.status(409).end('Package exists already.');
        ...
        logger.log('error', e)
        res.status(400).end('Error uploading package.');
    }
});
...
async (req: Request, res: Response) => {
    ...
    if (!content && !url) {
        ...
        res.status(400).end('Missing content and url.');
        return;
    }
    else if (content && url) {
        logger.log('info', 'Cannot have both content and url');
    ...
    try {
        ...
        if (!pkgData.metadata.url) {
            res.status(400).end('No url in package.json.');
        ...
        res.status(400).end('Failed to update package');
