...
export class LicenseMetric extends Metric {
	async get_metric(repo: Repository): Promise<GroupMetric> {
		let final_score = 0;
		const regex = new RegExp('((LGPL-2\.1)(\w+\b)?)|((LGPL-2\.0)(\w+\b)?)|((MIT)(\w+\b)?)');
		const license: string | null = await repo.get_license(); // ask for license file
		if (license == null) {
...
const getAllFiles = function (dirPath: string, fileArray: string[]) {
...
export class CorrectnessMetric extends Metric {
	private async get_eslint_on_clone(dir: string[]): Promise<LintResults | null> {
		...
		try {
			...
			let file_count_success = 0;
			let error_count_success = 0;
			let fixable_error_count_success = 0;
			let fatal_error_count_success = 0;
			let line_count_success: NullNum = 0;
	...
	async get_metric(repo: Repository): Promise<GroupMetric> {
		...
		const cloned_dir: string | null = await repo.get_local_clone("Correctness");
		let final_score: NullNum = 0.25;
		if (cloned_dir != null) {
			// do clone work here
			const array_of_files = getAllFiles(cloned_dir, []);
			let lint_results: LintResults | null;
			try {
...
export class VersionMetric extends Metric {
	async get_metric(repo: Repository): Promise<GroupMetric> {
		let score = 0; // subscore for readme
		const packages: any = await repo.get_package_json(); //get the readme
...
async function versionCalc(packages: any): Promise<number> {
	let score = 0; // subscore for readme
	let numberPinnedToMajorMinor = 0;
